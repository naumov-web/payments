...
class CreateSubscriptionUseCase:
    ...
    async def execute(
        ...
        *,
        ...
        if amount <= 0:
            raise InvalidSubscriptionError("Amount must be positive.")

        async with self._uow as uow:
            subscriber = await uow.actors.get_by_id(subscriber_actor_id)

            if subscriber is None:
                raise ActorNotFoundError("Subscriber not found.")

            service_actor = await uow.actors.get_by_id(service_actor_id)

            if service_actor is None:
                raise ActorNotFoundError("Service actor not found.")

            balance = await uow.wallet_balances.get_balance(subscriber_actor_id)
            ...
            if billing_period == BillingPeriod.WEEKLY:
                ...
                raise InvalidSubscriptionError("Unsupported billing period.")

            existing_subscription = await uow.subscriptions.get_active_subscription(
                subscriber_actor_id=subscriber_actor_id,
                service_actor_id=service_actor_id,
            )

            if existing_subscription is not None:
                raise SubscriptionAlreadyExistsError("Active subscription already exists.")
            ...
            try:
                await uow.subscriptions.create(subscription)
