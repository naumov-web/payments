...
class RefundTransferUseCase:
    REFUND_WINDOW = timedelta(minutes=10)
        ...
        *,
        ...
        async with self._uow as uow:
            ...
            if event.transaction_type != "TRANSFER":
                raise RefundNotAllowedError("Only transfers can be refunded.")
            ...
            request_hash = (
                build_transfer_request_hash(
                    sender_actor_id=receiver_entry.actor_id,
                    receiver_actor_id=sender_entry.actor_id,
                    amount=amount,
                )
            )
            ...
            receiver_balance = await uow.wallet_balances.get_balance(receiver_entry.actor_id)

            if receiver_balance < amount:
                raise InsufficientFundsError("Receiver does not have enough balance for refund.")
