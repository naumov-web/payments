...
WITHDRAWAL_SERVICE_ACTOR_ID = UUID("00000000-0000-0000-0000-000000000006")
...
class WithdrawalUseCase:
    ...
    async def execute(
        ...
        *,
        ...
        if amount <= 0:
            raise InvalidWithdrawalError("Amount must be positive.")

        request_hash = build_transfer_request_hash(
            sender_actor_id=actor_id,
            receiver_actor_id=WITHDRAWAL_SERVICE_ACTOR_ID,
            amount=amount,
        )
        ...
        async with self._uow as uow:
            ...
            actor = await uow.actors.get_by_id(actor_id)

            if actor is None:
                raise ActorNotFoundError("Actor not found.")

            withdrawal_service = await uow.actors.get_by_id(WITHDRAWAL_SERVICE_ACTOR_ID)

            if withdrawal_service is None:
                raise ActorNotFoundError("Withdrawal service actor not found.")

            balance = await uow.wallet_balances.get_balance(actor_id)
