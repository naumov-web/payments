    ...
    async def execute(
        ...
        *,
        ...
        if sender_actor_id == receiver_actor_id:
            raise InvalidTransferError("Sender and receiver must be different.")
        ...
        async with self._uow as uow:
            ...
            sender = await uow.actors.get_by_id(sender_actor_id)

            if sender is None:
                raise ActorNotFoundError("Sender actor not found.")

            receiver = await uow.actors.get_by_id(receiver_actor_id)

            if receiver is None:
                raise ActorNotFoundError("Receiver actor not found.")

            sender_balance = await uow.wallet_balances.get_balance(sender_actor_id)
            ...
            aggregate = TransactionAggregate.create_transfer(
                ...
                target_actor_id=receiver_actor_id,
                amount=amount,
            )
