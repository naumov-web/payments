...
class PurchaseUseCase:
    ...
    async def execute(
        ...
        *,
        ...
        if amount <= 0:
            raise InvalidPurchaseError("Amount must be positive.")

        if buyer_actor_id == merchant_actor_id:
            raise InvalidPurchaseError("Buyer and merchant must be different.")

        request_hash = build_transfer_request_hash(
            sender_actor_id=buyer_actor_id,
            receiver_actor_id=merchant_actor_id,
            amount=amount,
        )
        ...
        async with self._uow as uow:
            ...
            buyer = await uow.actors.get_by_id(buyer_actor_id)

            if buyer is None:
                raise ActorNotFoundError("Buyer actor not found.")

            merchant = await uow.actors.get_by_id(merchant_actor_id)

            if merchant is None:
                raise ActorNotFoundError("Merchant actor not found.")

            buyer_balance = await uow.wallet_balances.get_balance(buyer_actor_id)
